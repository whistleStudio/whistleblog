<script setup lang="ts">
  ...
  onMounted(() => {
    ...
    // 总线监听：静音app音乐
    let tim_app_mute: ReturnType<typeof setTimeout>
    bus.on("muteAppMusic", () => {
      commonHandles.updateBusSong(audioElement)
      clearTimeout(tim_app_mute)
      tim_app_mute = setTimeout(() => {isGlobalAudioMuted.value = true}, 80)
    })
  })

  function appPlayMusicHandle () {
    const audioElement = document.querySelector(".global-audio") as HTMLAudioElement;
    isGlobalAudioMuted.value = false
    commonHandles.updateViewSong(audioElement)
    if (bus.curSong.isPlaying) audioElement.play()
...
<template>
  <div class="app-box">
    <router-view @appPlayMusic="appPlayMusicHandle"></router-view>
  </div>
